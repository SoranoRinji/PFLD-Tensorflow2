...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        image_batch = tf.compat.v1.placeholder(tf.float32, shape=(None, args.image_size, args.image_size, 3), \
                                               name='image_batch')
        landmark_batch = tf.compat.v1.placeholder(tf.float32, shape=(None, 196), name='landmark_batch')
        attribute_batch = tf.compat.v1.placeholder(tf.int32, shape=(None, 6), name='attribute_batch')
        euler_angles_gt_batch = tf.compat.v1.placeholder(tf.float32, shape=(None, 3), name='euler_angles_gt_batch')
        ...
        attributes_w_n = tf.compat.v1.to_float(attribute_batch[:, 1:6])
        ...
        save_params = tf.compat.v1.trainable_variables()
        saver = tf.compat.v1.train.Saver(save_params, max_to_keep=None)
        gpu_options = tf.compat.v1.GPUOptions(per_process_gpu_memory_fraction=1.0)

        sess = tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpu_options, allow_soft_placement=False,
